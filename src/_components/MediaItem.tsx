...
export const MediaItem: FC<MediaItemProps> = ({ media, isLoading }) => {
  ...
  if (media.type === 'Photo') {
    ...
    const photoSizes = ['original', 'large2x', 'portrait', 'landscape']

    const dropDownData = Object.entries(media.src)
      .map(([key, value]) => ({
        key,
        value
      }))
      .filter((item) => photoSizes.includes(item.key))
    ...
    return (
      <div className="border relative select-none aspect-auto transition-all min-h-48 flex flex-col overflow-hidden rounded-xl">
        ...
        <MediaAuthor className="absolute left-0 bottom-0 w-full" media={media} dropDownData={dropDownData} />
      </div>
    )
  }
  ...
  if (media.type === 'Video') {
    ...
    const dropDownData = media.video_files
    ...
    return (
      ...
      >
        ...
        {/* @ts-expect-error TODO: fix this size field */}
        <MediaAuthor className="absolute left-0 bottom-0 w-full z-10" media={media} dropDownData={dropDownData} />
      </div>
    )
  }
}
